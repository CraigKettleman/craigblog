...
const AVATAR_SVG = `<svg width="256" height="256" viewBox="0 0 256 256" fill="none" xmlns="http://www.w3.org/2000/svg"><rect width="256" height="256" fill="#2C2C32"/><path d="M128 68C161.137 68 188 94.8629 188 128C188 144.678 181.194 159.766 170.209 170.641L127.568 128L85.3584 170.209C74.6274 159.369 68 144.459 68 128C68 94.8629 94.8629 68 128 68Z" fill="#FB923C"/><rect x="127.569" y="128" width="182.032" height="181.421" transform="rotate(45 127.569 128)" fill="#2C2C32"/></svg>`;
const AVATAR_DATA_URI = `data:image/svg+xml;base64,${Buffer.from(AVATAR_SVG).toString("base64")}`;
...
function brandingFooter(options: OgImageOptions, large?: boolean): Node {
  ...
  return h(
    ...
    h(
      ...
      options.brandTagline || "Personal Website",
    ),
  );
}
